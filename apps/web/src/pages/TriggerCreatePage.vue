...
<template>
  <div class="page-container form-page form-page-animated">
    <PageHeader
      title="创建触发器"
    ...
    <ui-card class="form-card">
      <ui-form layout="vertical">
        ...
        <!-- Token display -->
        <div v-if="lastToken" class="token-box">
          ...
          <code class="token-value">{{ lastToken }}</code>
          <p class="token-hint">注意：Token 只会显示一次，请妥善保管。</p>
        </div>

        <div class="form-actions">
          <ui-button @click="router.push(to.triggers.list())">{{ lastToken ? "返回列表" : "取消" }}</ui-button>
          <ui-button
            v-if="!lastToken"
            type="primary"
